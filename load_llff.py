...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    start = time.time()
    with open(f"{basedir}/train_set.pkl", "rb") as f:
        data = pkl.load(f)
    poses_arr = data["pb"]
    imgs_list = data["imgs"]
    ...
    # 这是啥意思，硬插进分辨率不是破坏了旋转矩阵？
    # 搞错了，旋转矩阵分为对坐标的旋转还是对坐标系的旋转，两者的旋转矩阵刚好相反的
...
# create render poses using training poses


def poses_interpolate(poses):

    hwf = poses[0, :3, -1:]
    train_poses = poses[:15]

    centers = train_poses[:, :3, 3]
    vec2s = train_poses[:, :3, 2]
    up = train_poses[:, :3, 1].sum(0)

    n_interpolate = 3
    render_poses = []
    for i in range(vec2s.shape[0]-1):
        delta_center = (centers[i+1] - centers[i])/n_interpolate
        delta_vec2 = (vec2s[i+1] - vec2s[i])/n_interpolate
        for j in range(3):
            center = centers[i] + (j * delta_center)
            vec2 = normalize(vec2s[i] + (j * delta_vec2))
            render_poses.append(np.concatenate(
...
def load_llff_data(basedir, factor=1, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, interpolate=True, gaze_num=0):
    ...
    if spherify:
        ...
        render_poses = poses_interpolate(poses)
